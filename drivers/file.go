// Package drivers contains cache store implementations.
// 包 drivers 包含缓存存储实现。
package drivers

import (
	"encoding/json"
	"fmt"
	"github.com/goal-web/contracts"
	"github.com/goal-web/supports/logs"
	"github.com/goal-web/supports/utils"
	"os"
	"path/filepath"
	"sync"
	"time"
)

// NewFile creates a new file-based cache instance.
// NewFile 创建新的基于文件的缓存实例。
func NewFile(config contracts.Fields) contracts.CacheStore {
	path := utils.GetStringField(config, "path", "./storage/cache")
	prefix := utils.GetStringField(config, "prefix", "")

	// Ensure cache directory exists.
	// 确保缓存目录存在。
	if err := os.MkdirAll(path, 0755); err != nil {
		panic(fmt.Errorf("failed to create cache directory: %w", err))
	}

	return &File{
		mutex:  sync.RWMutex{},
		path:   path,
		prefix: prefix,
	}
}

// cacheData represents cached data with expiration information.
// cacheData 表示带过期信息的缓存数据。
type cacheData struct {
	Value     any       `json:"value"`      // Value holds the cached value.
	                                         // Value 保存缓存值。
	ExpiredAt time.Time `json:"expired_at"` // ExpiredAt holds the expiration time.
	                                         // ExpiredAt 保存过期时间。
	Forever   bool      `json:"forever"`    // Forever indicates if the value never expires.
	                                         // Forever 指示值是否永不过期。
}

// File implements a file-based cache store.
// File 实现基于文件的缓存存储。
type File struct {
	mutex  sync.RWMutex  // mutex provides thread-safe access to the file cache.
	                     // mutex 提供对文件缓存的线程安全访问。
	path   string        // path holds the cache directory path.
	                     // path 保存缓存目录路径。
	prefix string        // prefix holds the key prefix for this store.
	                     // prefix 保存此存储的键前缀。
}

// getFilePath returns the file path for the given key.
// getFilePath 返回给定键的文件路径。
func (f *File) getFilePath(key string) string {
	// Use MD5 hash as file name to avoid special character issues.
	// 使用MD5哈希作为文件名，避免特殊字符问题。
	hashedKey := utils.Md5(f.prefix + key)
	return filepath.Join(f.path, hashedKey+".cache")
}

// readCacheData reads and unmarshals cache data from the file.
// readCacheData 从文件中读取并反序列化缓存数据。
func (f *File) readCacheData(filePath string) (*cacheData, error) {
	data, err := os.ReadFile(filePath)
	if err != nil {
		return nil, err
	}

	var cacheData cacheData
	if err := json.Unmarshal(data, &cacheData); err != nil {
		return nil, err
	}

	return &cacheData, nil
}

// writeCacheData marshals and writes cache data to the file.
// writeCacheData 序列化并将缓存数据写入文件。
func (f *File) writeCacheData(filePath string, data *cacheData) error {
	jsonData, err := json.Marshal(data)
	if err != nil {
		return err
	}

	// Ensure directory exists.
	// 确保目录存在。
	dir := filepath.Dir(filePath)
	if err := os.MkdirAll(dir, 0755); err != nil {
		return err
	}

	return os.WriteFile(filePath, jsonData, 0644)
}

// isExpired checks if the cache data has expired.
// isExpired 检查缓存数据是否已过期。
func (f *File) isExpired(data *cacheData) bool {
	return !data.Forever && time.Now().After(data.ExpiredAt)
}

// Get retrieves a value from the cache by key.
// Get 根据键从缓存中获取值。
func (f *File) Get(key string) any {
	f.mutex.RLock()
	defer f.mutex.RUnlock()

	filePath := f.getFilePath(key)
	if !f.fileExists(filePath) {
		return nil
	}

	data, err := f.readCacheData(filePath)
	if err != nil {
		logs.WithError(err).WithField("key", key).Debug("cache.File.Get: failed to read cache data")
		return nil
	}

	if f.isExpired(data) {
		// Delete expired file.
		// 删除过期文件。
		os.Remove(filePath)
		return nil
	}

	return data.Value
}

// Many retrieves multiple values from the cache by keys.
// Many 根据键列表从缓存中获取多个值。
func (f *File) Many(keys []string) []any {
	f.mutex.RLock()
	defer f.mutex.RUnlock()

	results := make([]any, len(keys))
	for i, key := range keys {
		results[i] = f.Get(key)
	}
	return results
}

// Put stores a value in the cache with an expiration time.
// Put 将带过期时间的值存储在缓存中。
func (f *File) Put(key string, value any, seconds time.Duration) error {
	f.mutex.Lock()
	defer f.mutex.Unlock()

	data := &cacheData{
		Value:     value,
		ExpiredAt: time.Now().Add(seconds),
		Forever:   false,
	}

	filePath := f.getFilePath(key)
	return f.writeCacheData(filePath, data)
}

// Add adds a value to the cache with an expiration time if it doesn't exist.
// Add 如果不存在则将带过期时间的值添加到缓存中。
func (f *File) Add(key string, value any, ttl ...time.Duration) bool {
	// Check if already exists.
	// 检查是否已存在。
	if f.Get(key) != nil {
		return false
	}

	lifetime := time.Second * 5
	if len(ttl) > 0 {
		lifetime = ttl[0]
	}

	return f.Put(key, value, lifetime) == nil
}

// Pull retrieves and removes a value from the cache by key.
// Pull 根据键从缓存中获取并删除值。
func (f *File) Pull(key string, defaultValue ...any) any {
	value := f.Get(key)
	if value == nil {
		return utils.DefaultInterface(defaultValue)
	}

	// Delete cache item.
	// 删除缓存项。
	f.Forget(key)
	return value
}

// PutMany stores multiple values in the cache with an expiration time.
// PutMany 将多个带过期时间的值存储在缓存中。
func (f *File) PutMany(values map[string]any, seconds time.Duration) error {
	f.mutex.Lock()
	defer f.mutex.Unlock()

	now := time.Now()
	for key, value := range values {
		data := &cacheData{
			Value:     value,
			ExpiredAt: now.Add(seconds),
			Forever:   false,
		}

		filePath := f.getFilePath(key)
		if err := f.writeCacheData(filePath, data); err != nil {
			return err
		}
	}

	return nil
}

// Increment increments the value of the given key by the given value.
// Increment 按给定值增加指定键的值。
func (f *File) Increment(key string, value ...int64) (int64, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()

	increment := utils.DefaultInt64(value, 1)
	currentValue := f.Get(key)

	var count int64
	if currentValue == nil {
		count = increment
	} else {
		count = utils.ToInt64(currentValue, 0) + increment
	}

	// Use default TTL.
	// 使用默认TTL。
	if err := f.Put(key, count, 24*time.Hour); err != nil {
		return 0, err
	}

	return count, nil
}

// Decrement decrements the value of the given key by the given value.
// Decrement 按给定值减少指定键的值。
func (f *File) Decrement(key string, value ...int64) (int64, error) {
	f.mutex.Lock()
	defer f.mutex.Unlock()

	decrement := utils.DefaultInt64(value, 1)
	currentValue := f.Get(key)

	var count int64
	if currentValue == nil {
		count = -decrement
	} else {
		count = utils.ToInt64(currentValue, 0) - decrement
	}

	// Use default TTL.
	// 使用默认TTL。
	if err := f.Put(key, count, 24*time.Hour); err != nil {
		return 0, err
	}

	return count, nil
}

// Forever stores a value in the cache without expiration time.
// Forever 将无过期时间的值存储在缓存中。
func (f *File) Forever(key string, value any) error {
	f.mutex.Lock()
	defer f.mutex.Unlock()

	data := &cacheData{
		Value:   value,
		Forever: true,
	}

	filePath := f.getFilePath(key)
	return f.writeCacheData(filePath, data)
}

// Forget removes a value from the cache by key.
// Forget 根据键从缓存中删除值。
func (f *File) Forget(key string) error {
	f.mutex.Lock()
	defer f.mutex.Unlock()

	filePath := f.getFilePath(key)
	if f.fileExists(filePath) {
		return os.Remove(filePath)
	}
	return DataNotExistsErr
}

// Flush flushes all values from the cache.
// Flush 清空缓存中的所有值。
func (f *File) Flush() error {
	f.mutex.Lock()
	defer f.mutex.Unlock()

	// Delete all cache files.
	// 删除所有缓存文件。
	return os.RemoveAll(f.path)
}

// GetPrefix returns the cache key prefix.
// GetPrefix 返回缓存键前缀。
func (f *File) GetPrefix() string {
	return f.prefix
}

// Remember retrieves a value from the cache or executes the provider to set it.
// Remember 从缓存中获取值，如果不存在则执行提供者设置值。
func (f *File) Remember(key string, ttl time.Duration, provider contracts.InstanceProvider[any]) any {
	if value := f.Get(key); value != nil {
		return value
	}

	value := provider()
	if err := f.Put(key, value, ttl); err != nil {
		logs.WithError(err).WithField("value", value).Debug("cache.File.Remember: value put failed")
	}

	return value
}

// RememberForever retrieves a value from the cache or executes the provider to set it without expiration.
// RememberForever 从缓存中获取值，如果不存在则执行提供者设置值，无过期时间。
func (f *File) RememberForever(key string, provider contracts.InstanceProvider[any]) any {
	if value := f.Get(key); value != nil {
		return value
	}

	value := provider()
	if err := f.Forever(key, value); err != nil {
		logs.WithError(err).WithField("value", value).Debug("cache.File.RememberForever: value put failed")
	}

	return value
}

// fileExists checks if a file exists.
// fileExists 检查文件是否存在。
func (f *File) fileExists(filePath string) bool {
	_, err := os.Stat(filePath)
	return !os.IsNotExist(err)
}
